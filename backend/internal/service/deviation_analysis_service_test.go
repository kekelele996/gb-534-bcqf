...
func TestAnalysisIdempotencyReviewerSeparationAndReplay(t *testing.T) {
	...
	svc := NewDeviationAnalysisService(analysisRepo, recipeRepo, seriesRepo, auditRepo, repository.NewPhaseReviewRepository(db), algorithm.NewEvaluator())
...
func TestRunRequiresReadySeriesAndIdempotencyKey(t *testing.T) {
	...
	svc := NewDeviationAnalysisService(
		...
		repository.NewSensorSeriesRepository(db), repository.NewAuditRepository(db),
		repository.NewPhaseReviewRepository(db), algorithm.NewEvaluator(),
	)
