...
- 高风险阶段逐项复核：加权偏差 ≥20% 的阶段必须逐个由复核人处理，可认可证据、排除具体疑似原因或退回调查；退回与排除必须填写依据，阶段由首个复核人接收锁定，全部阶段给出非退回结论后才允许确认。
...
| `POST` | `/api/v1/deviation-analyses/:id/phase-reviews` | 提交单个高风险阶段的复核结论（认可/排除/退回） |
...
高风险阶段复核规则：任一阶段加权偏差达到 20% 即列入必处理清单，复核人可对单个阶段提交「认可证据」「排除疑似原因」「退回调查」；排除必须从该阶段命中的疑似原因中选择具体原因，退回和排除都必须填写依据。阶段由首个提交的复核人接收（数据库唯一约束保证），之后其他复核人提交会收到 409「已被接收」，仅接收人可修改结论；任何阶段无结论或处于退回调查状态时确认会被 409 挡住，响应同时列出待处理项。每次提交都写审计日志，记录操作者、操作时间、前后结论和依据。
